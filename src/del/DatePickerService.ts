...
// // повторные фильтры будутсрабатывать в течение этого времени
...
//     Promise.all(
//       this._model.dimensions.map((dim) =>
//         fetch(`api/v3/koob/${this._model.koob}.${dim}`).then((resp) =>
//           resp.json()
//         )
//       )
//     ).then((responses) => {
...
//         with: this._model.koob,
//         columns: this._model.dimensions,
//         sort: ["+period_name", "+period", "+scenario"],
//         distinct: [],
//       }).then((dataPeriods) => {
//         let data = [];
//         dataPeriods?.map((dataElement) => {
//           let currentPeriodType = data.find(
//             (el) => el.id == dataElement.period_name
//           );
//           const currentPeriodName_name =
//             period_names_dict.find((el) => el.id == dataElement.period_name) ||
//             "";
//           const currentPeriod_name =
//             periods_dict.find((el) => el.id == dataElement.period) || "";
//           const currentScenario_name =
//             scenarios_dict.find((el) => el.id == dataElement.scenario) || "";
//           const currentYear =
//             dataElement.period_name == 1
//               ? currentPeriod_name.title
//               : currentPeriod_name.title.split("_")[1];
...
//               title:
//                 dataElement.period_name == 1
//                   ? currentScenario_name.title
//                   : dataElement.period_name == 2
//                   ? QUARTERS[periodSplitted[0]]
//                   : MONTHS[periodSplitted[0]],
...
//             } else {
//               const currentElement = currentPeriodType.elements.find(
//                 (el) => el.year == newChild.year && el.title == newChild.title
//               );
//               if (!currentElement) {
//                 currentPeriodType.elements.push(newChild);
//               }
...
//         const currentYearFrom = years.includes(currentDateYear)
//           ? currentDateYear
//           : years[0];
//         const currentYearTo = years.includes(currentDateYear)
//           ? currentDateYear
//           : years[years.length - 1];
//         const currentScenarioFrom = 1;
//         const currentMonthIndex = Number(currentDateMonthIndex) + 1;
//         const currentPeriodFrom = uniq(
//           data
//             .find((el) => el.id == 1)
//             .elements.filter(
//               (el) => el.scenario == 1 && el.year == currentYearFrom
//             )
//             .map((el) => el.period)
//         );
//         const currentPeriodTo = uniq(
//           data
//             .find((el) => el.id == 1)
//             .elements.filter(
//               (el) =>
//                 el.scenario == currentMonthIndex && el.year == currentYearTo
//             )
//             .map((el) => el.period)
//         );
//         this._updateWithData({
//           period_names_dict: period_names_dict,
...
//           currentPeriodTo,
//           currentScenarioFrom,
//           currentScenarioTo: currentMonthIndex,
//         });
//       });
//     });
//   }
...
//                     : currentPeriodTypeTo)
//               )
//               ?.elements.find((el) => el.year == id)?.year
//           : "";
//         break;
//       case "period":
//       case "scenario":
...
//                     : currentPeriodTypeTo)
//               )
//               ?.elements.find((el) => el[type] == id)?.title
//           : "";
//         break;
//     }
//     return title;
...
//       position == "from" ? currentPeriodTypeFrom : currentPeriodTypeTo;
//     const currentYear = position == "from" ? currentYearFrom : currentYearTo;
//     const currentPeriodIds =
//       position == "from" ? currentPeriodFrom : currentPeriodTo;
//     const currentScenario =
//       position == "from" ? currentScenarioFrom : currentScenarioTo;
//     let titles =
//       data
//         .find((el) => el.id == currentPeriodType)
//         ?.elements.filter(
//           (el) =>
//             String(el.year) == String(currentYear) &&
//             currentPeriodIds.includes(el.period)
//         ) || [];
//     if (currentPeriodType == 1) {
//       titles = titles?.filter((el) => el.scenario == currentScenario);
//     }
//     const currentPeriodElementsTitles = titles
//       ?.filter((el) => el.title)
//       .map((el) => el.title);

//     return `${this.getBranchTitleById("year", currentYear, position)} ${
//       currentPeriodType == 1
//         ? titles?.[0]?.title
//         : currentPeriodElementsTitles.length == 1
//         ? currentPeriodElementsTitles[0]
//         : `${position == "from" ? "Базовый период" : "Отчетный период"}`
//     }`;
//   }
...
//       window.__datePickerService[String(this.id)]
//     ) {
...
//       window.__datePickerService[String(id)] = new DatePickerService(
//         String(id)
//       );
//     }
